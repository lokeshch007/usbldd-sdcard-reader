...
unsigned char SD_sendCommand(unsigned char cmd, uint32_t arg)
{
    ...
    ssp_write(cmd | 0x40); //send command, first two bits always '01'
   	ssp_write(arg>>24);
  	ssp_write(arg>>16);
    ssp_write(arg>>8);
    ssp_write(arg);
    ...
    if(cmd == SEND_IF_COND)	 //it is compulsory to send correct CRC for CMD8 (CRC=0x87) & CMD0 (CRC=0x95)
        ssp_write(0x87);    //for remaining commands, CRC is ignored in SPI mode
    else
        ssp_write(0x95);

    while((response = ssp_read()) == 0xff) //wait response
        if(retry++ > 0xfe) break; //time out error

    if(response == 0x00 && cmd == 58)  //checking response of CMD58
    {
        status = ssp_read() & 0x40;     //first byte of the OCR register (bit 31:24)
        if(status == 0x40)
        {
        ...
        ssp_read(); //remaining 3 bytes of the OCR register are ignored here
        ssp_read(); //one can use these bytes to check power supply limits of SD
        ssp_read();
    }

    ssp_read(); //extra 8 CLK
    SSP_DISABLE_SLAVE();
...
unsigned char SD_readSingleBlock(char *inputbuffer,uint32_t startBlock)
{
    ...
    retry = 0;
    while(ssp_read() != 0xfe) //wait for start block token 0xfe (0x11111110)
    {
        if(retry++ > 0xfffe)
        {
            SSP_DISABLE_SLAVE();
    ...
    for(i=0; i<512; i++) //read 512 bytes
        inputbuffer[i] = ssp_read();

    ssp_read(); //receive incoming CRC (16-bit), CRC is ignored here
    ssp_read();

    ssp_read(); //extra 8 clock pulses
    SSP_DISABLE_SLAVE();
...
unsigned char SD_writeSingleBlock(char *inputbuffer,uint32_t startBlock)
{
    ...
    ssp_write(0xfe);     //Send start block token 0xfe (0x11111110)

    for(i=0; i<512; i++)    //send 512 bytes data
        ssp_write(inputbuffer[i]);

    ssp_write(0xff);     //transmit dummy CRC (16-bit), CRC is ignored here
    ssp_write(0xff);
    ...
    {                              //AAA='101'-data rejected due to CRC error
        SSP_DISABLE_SLAVE();              //AAA='110'-data rejected due to write error
        return response;
    }

    while(!ssp_read()) //wait for SD card to complete writing and get idle
    {
        if(retry++ > 0xfffe)
        {
            SSP_DISABLE_SLAVE();
    ...
    SSP_DISABLE_SLAVE();
    ssp_write(0xff);   //just spend 8 clock cycle delay before reasserting the CS line
    SSP_DISABLE_SLAVE();         //re-asserting the CS line to verify if card is still busy
...
uint8_t init_SdCard(uint8_t *cardType)
{
    ...
    for(i=0;i<10;i++)
       	ssp_write(0xff);   //80 clock pulses spent before sending the first command

	//SLAVE(SD Card) is enabled
    SSP_ENABLE_SLAVE();
    do
    {
