                ...
                *fileOpenSts = openFile (VERIFY, filePtr);
                ...
                else if(*fileOpenSts == FAT32_FILE_DELETED_OR_NOT_FOUND)
                {
                    *fileOpenSts = FAT32_FILE_OPEN_SUCCESSFUL;
                    ...
                    getSetNextCluster(filePtr->fileBuffer,filePtr->cluster, SET_FAT, END_OF_CLUSTERS);
 ...
 * description :
                 ...
                 Once the End of File is reached it returns EOF_FAT(26). User shoukd stop calling the function after this.
 ******************************************************************************************************************/
char FILE_GetCh(fileConfig_st *filePtr)
{
    char ch;
...
void FILE_PutCh (fileConfig_st *filePtr, char data)
{
    ...
    {
        if((filePtr->endOfFileDetected == 0) && (data != EOF_FAT))
        {
            ...
            {               //infinite loop in case it happens to be greater than 512 due to some data corruption
                ...
                if(filePtr->sectorIndex == sectorPerCluster)
                {
                    ...
                    getSetNextCluster(filePtr->fileBuffer,filePtr->prevCluster, SET_FAT, filePtr->cluster);
                    getSetNextCluster(filePtr->fileBuffer,filePtr->cluster, SET_FAT, END_OF_CLUSTERS);   //last cluster of the file, marked END_OF_CLUSTERS
        ...
        else if(filePtr->endOfFileDetected == 0)
        {
            ...
            getSetFreeCluster (filePtr->fileBuffer,NEXT_FREE, SET_FAT, filePtr->cluster); //update FSinfo next free cluster entry
            ...
            while(1)
            {
                ...
                if(filePtr->cluster > 0x0ffffff6)
                {
                    if(filePtr->cluster == END_OF_CLUSTERS)   //this situation will come when total files in root is multiple of (32*sectorPerCluster)
                    {
                        ...
                        getSetNextCluster(filePtr->fileBuffer,filePtr->prevCluster, SET_FAT, filePtr->cluster); //link the new cluster of root to the previous cluster
                        getSetNextCluster(filePtr->fileBuffer,filePtr->cluster, SET_FAT, END_OF_CLUSTERS);  //set the new cluster as end of the root directory
                    }
                    else
                    {
...
//Arguments: 1. current cluster number, 2. get_set (=GET, if next cluster is to be found or = SET_FAT,
...
//			 2.flag: GET or SET_FAT 
//			 3.new FS entry, when argument2 is SET_FAT; or 0, when argument2 is GET
//return: next free cluster, if arg1 is NEXT_FREE & arg2 is GET
//        total number of free clusters, if arg1 is TOTAL_FREE & arg2 is GET
//		  0xffffffff, if any error or if arg2 is SET_FAT
//********************************************************************************************
uint32_t getSetFreeCluster(char *inputBuffer, uint8_t totOrNext, uint8_t get_set, uint32_t FSEntry)
{
...
void freeMemoryUpdate (char *inputBuffer, uint8_t flag, uint32_t size)
{
    ...
    if(freeClusterCountUpdated)
    {
        ...
        if(flag == ADD)
            ...
            freeClusters = freeClusters - size;
        getSetFreeCluster (inputBuffer,TOTAL_FREE, SET_FAT, freeClusters);
    }
}
                ...
                if((dir->name[0] != DELETED) && (dir->attrib != ATTR_LONG_NAME))
                {
                    ...
                    if(j == 11)
                    {
                        ...
                        if(firstCluster < cluster)
                            getSetFreeCluster (GlobalBuffer,NEXT_FREE, SET_FAT, firstCluster);

                        //mark all the clusters allocated to the file as 'free'
                        while(1)
                        {
                            nextCluster = getSetNextCluster(GlobalBuffer,firstCluster, GET, 0);
                            getSetNextCluster(GlobalBuffer,firstCluster, SET_FAT, 0);
